...
C     
      ...
      DOUBLE PRECISION XPSI1
      DOUBLE PRECISION XPSIBAR2
...
C     Continue only if IMODE is 0, 4 or 5
      ...
      IF (ABS(LPP(IB(1))).GE.1) THEN
          !LP=SIGN(1,LPP(IB(1)))
        XPSI1=PDG2PDF(LPP(IB(1)),51, IB(1),XBK(IB(1)),DSQRT(Q2FACT(1)))
      ENDIF
      IF (ABS(LPP(IB(2))).GE.1) THEN
          !LP=SIGN(1,LPP(IB(2)))
        XPSIBAR2=PDG2PDF(LPP(IB(2)),-51, IB(2),XBK(IB(2))
     $   ,DSQRT(Q2FACT(2)))
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! ~psi ~psibar > b b~ ta- ta+
      PD(IPROC)=XPSI1*XPSIBAR2
